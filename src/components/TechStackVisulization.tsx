...
export function TechStackVisualization() {
  ...
  // helper: percent -> px (fallback to 0 if size unknown)
  const px = (percent: number, axis: 'x' | 'y') => {
    if (axis === 'x') return size.width ? (percent / 100) * size.width : 0;
    return size.height ? (percent / 100) * size.height : 0;
  };
  ...
  style={{
    width: '100%',
    height: '100vh',        // <- guaranteed height
    position: 'relative',
    background: 'black',
    overflow: 'hidden',
  }}
>  {/* Animated background grid */}
      ...
      {particles.map((particle) => (
        <motion.div
          ...
          animate={{ y: [0, -100, 0], x: [0, Math.sin(particle.id) * 50, 0], opacity: [0.2, 0.8, 0.2] }}
          transition={{ duration: particle.duration, repeat: Infinity, delay: particle.delay, ease: 'easeInOut' }}
        />
      ))}

      {/* Main content (svg + nodes) */}
      <div className="relative w-full h-full flex items-center justify-center" style={{ zIndex: 10 }}>
        <svg className="absolute inset-0 w-full h-full" style={{ pointerEvents: 'none' }}>
          ...
          {connections.map((conn, idx) => {
            ...
            // numeric px coordinates
            const x1 = px(from.x, 'x');
            const y1 = px(from.y, 'y');
            const x2 = px(to.x, 'x');
            const y2 = px(to.y, 'y');

            // If we don't have measured size yet, skip animated dots to avoid NaN/undefined errors
            const canAnimateDot = size.width > 0 && size.height > 0;
            ...
            return (
              <g key={`${conn.from}-${conn.to}`}>
                <motion.line
                  ...
                  animate={{ pathLength: 1, opacity: activeNode === conn.from || activeNode === conn.to ? 1 : 0.4 }}
                  transition={{ pathLength: { duration: 2, delay: idx * 0.08 }, opacity: { duration: 0.25 } }}
        ...
        {/* Tech nodes — absolutely positioned over the svg */}
        {techs.map((tech, idx) => (
          <motion.div
            key={tech.id}
          ...
          >
            {/* Node visuals (slightly reduced sizes for better layout) */}
            <div
            ...
            >
              ...
              <div className="mt-2 text-xs tracking-wider uppercase" style={{ color: tech.color }}>{tech.name}</div>
            </div>
          </motion.div>
        ))}
      </div>
      ...
      {/* Title */}
      <motion.div className="absolute top-8 left-1/2 transform -translate-x-1/2 text-center" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 1 }}>
        <h1 className="text-5xl tracking-wider uppercase bg-gradient-to-r from-cyan-400 via-blue-400 to-sky-500 bg-clip-text text-transparent" style={{ textShadow: '0 0 40px rgba(14, 165, 233, 0.3)' }}>
          Tech Stack
        </h1>
      </motion.div>

      {/* scanline */}
      <div className="absolute inset-0 pointer-events-none bg-[linear-gradient(transparent_50%,rgba(14,165,233,0.03)_50%)] bg-[length:100%_4px] animate-[scan_8s_linear_infinite]" style={{ zIndex: 5 }} />
